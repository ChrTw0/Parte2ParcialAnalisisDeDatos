#!/usr/bin/env python3
"""
Extractor de JSONs normalizados a CSV/Excel - SOLO TASAS
Filtra únicamente items clasificados como TASA
"""
import json
import csv
...
# Configuración
OUTPUT_CSV = OUTPUT_DIR / "tarifarios_bancarios_SOLO_TASAS.csv"
OUTPUT_EXCEL = OUTPUT_DIR / "tarifarios_bancarios_SOLO_TASAS.xlsx"


def extract_items_from_json(json_file: Path, banco_name: str) -> List[Dict[str, Any]]:
    """
    Extrae SOLO items de tipo TASA de un JSON normalizado
    ...
    try:
        ...
        for documento in data.get('documentos', []):
            ...
            for item in documento.get('items', []):
                ...
                # ⭐ FILTRO: Solo procesar items de tipo TASA
                if tipo != 'TASA':
                    continue
                ...
                # Extraer tasas porcentuales MN
                if valores and valores.get('mn'):
                    mn = valores['mn']
                    tasa_pct_mn = mn.get('tasa_porcentaje')

                # Extraer tasas porcentuales ME
                if valores and valores.get('me'):
                    me = valores['me']
                    tasa_pct_me = me.get('tasa_porcentaje')

                # Si no tiene tasa porcentual en ninguna moneda, saltar
                if tasa_pct_mn is None and tasa_pct_me is None:
                    continue
        ...
        logger.debug(f"  Extraídos {len(items_extraidos)} tasas de {json_file.name}")
...
def main():
    logger.info("=" * 70)
    logger.info("📊 EXTRACCIÓN DE JSONs A CSV/EXCEL - SOLO TASAS")
    logger.info("=" * 70)
    ...
    logger.info("\n🔍 Procesando archivos JSON (filtrando solo TASAS)...")
    ...
    for banco_dir in sorted(JSON_DIR.iterdir()):
        ...
        logger.success(f"  ✅ Extraídas {banco_items} tasas de {banco_name}")

    # Verificar que se extrajeron datos
    if not all_items:
        logger.error("❌ No se extrajeron tasas. Verifica los archivos JSON.")
        return

    logger.info(f"\n📊 Total tasas extraídas: {len(all_items)}")
    ...
    with pd.ExcelWriter(OUTPUT_EXCEL, engine='openpyxl') as writer:
        # Hoja principal con todas las tasas
        df.to_excel(writer, sheet_name='Tasas', index=False)

        # Hoja de resumen por banco
        resumen_df = pd.DataFrame([
            {'Banco': banco, 'Total_Tasas': count}
            for banco, count in sorted(bancos_stats.items())
        ])
        resumen_df.to_excel(writer, sheet_name='Resumen_Por_Banco', index=False)

        # Hoja de resumen por categoría
        if 'Categoria' in df.columns and df['Categoria'].notna().any():
            categoria_df = df.groupby('Categoria').size().reset_index(name='Cantidad')
            categoria_df.to_excel(writer, sheet_name='Por_Categoria', index=False)

        # Hoja de tasas más altas MN
        df_mn = df[df['Tasa_Porcentaje_MN'].notna()].copy()
        if not df_mn.empty:
            top_mn = df_mn.nlargest(20, 'Tasa_Porcentaje_MN')[
                ['Banco', 'Producto_Nombre', 'Concepto', 'Tasa_Porcentaje_MN', 'Fecha_Vigencia']
            ]
            top_mn.to_excel(writer, sheet_name='Top_20_Tasas_MN', index=False)

        # Hoja de tasas más altas ME
        df_me = df[df['Tasa_Porcentaje_ME'].notna()].copy()
        if not df_me.empty:
            top_me = df_me.nlargest(20, 'Tasa_Porcentaje_ME')[
                ['Banco', 'Producto_Nombre', 'Concepto', 'Tasa_Porcentaje_ME', 'Fecha_Vigencia']
            ]
            top_me.to_excel(writer, sheet_name='Top_20_Tasas_ME', index=False)
    ...
    # Resumen final
    logger.info("\n" + "=" * 70)
    logger.info("📊 RESUMEN FINAL - SOLO TASAS")
    logger.info("=" * 70)
    logger.info(f"Total tasas extraídas:     {len(all_items)}")
    logger.info(f"Total bancos procesados:   {len(bancos_stats)}")
    logger.info("")
    logger.info("Por banco:")
    for banco, count in sorted(bancos_stats.items()):
        logger.info(f"  {banco:30} {count:4} tasas")
    logger.info("")

    # Estadísticas de tasas
    if not df.empty:
        logger.info("Estadísticas de tasas:")

        if df['Tasa_Porcentaje_MN'].notna().any():
            logger.info(f"  Tasas MN:")
            logger.info(f"    - Promedio: {df['Tasa_Porcentaje_MN'].mean():.2f}%")
            logger.info(f"    - Mínima:   {df['Tasa_Porcentaje_MN'].min():.2f}%")
            logger.info(f"    - Máxima:   {df['Tasa_Porcentaje_MN'].max():.2f}%")

        if df['Tasa_Porcentaje_ME'].notna().any():
            logger.info(f"  Tasas ME:")
            logger.info(f"    - Promedio: {df['Tasa_Porcentaje_ME'].mean():.2f}%")
            logger.info(f"    - Mínima:   {df['Tasa_Porcentaje_ME'].min():.2f}%")
            logger.info(f"    - Máxima:   {df['Tasa_Porcentaje_ME'].max():.2f}%")

    logger.info("=" * 70)

    logger.success("\n✅ Extracción de TASAS completada exitosamente")
